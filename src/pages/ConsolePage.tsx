...
export function ConsolePage() {
    ...
    const chatflowId = localStorage.getItem('tmp::chatflow_id') ||
        prompt('Chatflow ID') || ''
    if (chatflowId !== '') {
        localStorage.setItem('tmp::chatflow_id', chatflowId)
    }
    ...
    // 呼叫你自己的 Agent，傳入使用者轉寫文字，回傳文字答案
    const callAgent = useCallback(async (userText: string): Promise<string> => {
        try {
            // TODO: 將此 URL 改成你自己的 Agent 端點
            const payload = {
                question: userText,            // 使用者的問題/文字
                streaming: false,             // 若要串流回傳可改 true
                // 可選：帶上會話/追蹤資訊
                chatId: sessionID,
                // 可選：覆寫 Flowise 節點設定
                overrideConfig: {
                ...
                // 可選：你自己的自訂資料
                metadata: {
                    source: 'realtime-console',
                    timestamp: Date.now()
                }
            }

            const res = await fetch(`${FLOWISE_BASE_URL}/api/v1/prediction/${chatflowId}`, {
                method: 'POST',
                headers: {
            ...
            if (!res.ok) {
                console.error('Agent API error status:', res.status)
                return ''
            }
     ...
     * change chatflowId
     */
    const resetChatflowId = useCallback(() => {
        localStorage.setItem('tmp::chatflow_id' , '')
    ...
    useEffect(() => {
        ...
        client.on('conversation.updated', async ({ item, delta }: any) => {
            const returnItems: any[] = client.conversation.getItems()
            const items: any[] = []
            ...
            if (delta?.audio && items.includes(item)) {
            ...
            if (item.status === 'completed' && item.formatted.audio?.length) {
                ...
                if (item.role === 'user' && delta?.transcript) {
                    const answer = await callAgent(delta.transcript)
    ...
    return (
        <div data-component="ConsolePage">
            <div className="content-top">
                ...
                <div className="content-chatflow-id">
                    <Button
                        icon={Edit}
                        iconPosition="end"
                        buttonStyle="flush"
                        label={`CHATFLOW ID: ${chatflowId}`}
                        onClick={() => resetChatflowId()}
                    />
                </div>
            </div>
